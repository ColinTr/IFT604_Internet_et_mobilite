...
exports.getSoldes = async (req, res) => {
  soldeService
    .getSoldesFromDashboard(config.MONGODB_DASHBOARD_ID)
    .then((response) => {
    ...
    .catch((err) => {
      ...
      res
        .status(400)
        .send(new Erreur("Impossible de trouver la liste de solde"));
    });
};

exports.getTransactions = async (req, res) => {
  transactionService
    .getTransactionsFromDashboard(config.MONGODB_DASHBOARD_ID)
    .then((response) => {
      logger.info(response);
      res.send(response);
    })
    .catch((err) => {
      ...
      res
        .status(400)
        .send(new Erreur("Impossible de trouver la liste de transaction"));
    });
};
...
exports.createTransaction = async (req, res) => {
  if (
    ...
    logger.error(
      "Contenu manquant dans le body de la requête d'ajout de transaction."
    );
    return res
      .status(400)
      .send(
    ...
    try {
      ...
      if (transaction === undefined) {
        return res
          .status(502)
          .send(new Erreur("Le serveur n'a pas réussi à créer la transaction"));
      }
      ...
      if (resFrom === undefined || resTo.includes(undefined)) {
        return res
          ...
          .send(
            new Erreur(
              "Something went wrong during the process of your request"
            )
          );
      }
      ...
      return res
        .status(502)
        .send(
          new Erreur("Something went wrong during the process of your request")
        );
    }
  }
};
...
exports.deleteTransaction = async (req, res) => {
  if (req.params.id === undefined) {
    ...
    try {
      ...
      const trIsDeleted = await transactionService.removeTransaction(
        req.params.id
      );
      ...
      if (trIsDeleted === undefined) {
        return res
          .status(502)
          .send(
            new Erreur("Le serveur n'a pas réussi à supprimer la transaction")
          );
      }
