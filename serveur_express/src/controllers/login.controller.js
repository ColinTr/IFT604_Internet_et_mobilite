  ...
  try {
    ...
    if (req.headers.authorization === undefined) {
      ...
      return res
        ...
        .send(
          '{"redirectUrl":"' +
            google_utils.getConnectionUrl(google_utils.createConnection()) +
            '"}'
        );
    }
...
exports.getLogin = async (req, res, next) => {
  const google_account = await google_utils.getGoogleEmailAndTokensFromCode(
    req.query.code
  );
  ...
  UserService.getUsersByGoogleEmail(google_account.email)
    .then((user) => {
      if (user !== null) {
        ...
        UserService.addUser(
          google_account.email.split("@")[0],
          "no_need",
          google_account.tokens.access_token,
          google_account.tokens.refresh_token,
  ...
  return res
    .status(200)
    .redirect(
      "http://localhost:3000/completeAuthentication?tokens=" +
        JSON.stringify(google_account.tokens) +
        "&email=" +
        JSON.stringify(google_account.email)
    );
};
